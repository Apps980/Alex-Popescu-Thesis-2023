...
```{r BOUT MODEL 1}
BOUT.MOD1<-lmer(LDURATION~BEHAVIOR+SENTINEL_PRESENCE+GENERALIZED_ENVIRONMENT+BAIT_PRESENCE+GROUP_SIZE+DISTURBANCE_FREQUENCY+(1|ID), data = BOUT)
...
```{r BOUT MODEL 2}
BOUT.MOD2<-lmer(LDURATION~BEHAVIOR*SENTINEL_PRESENCE+BEHAVIOR*GENERALIZED_ENVIRONMENT+SENTINEL_PRESENCE*GENERALIZED_ENVIRONMENT+BEHAVIOR*GROUP_SIZE+BEHAVIOR*BAIT_PRESENCE+DISTURBANCE_FREQUENCY+(1|ID)+(1|VIDEO_ID), data = BOUT)
...
```{r RLMM MOD 2}
BOUT.RMOD2<-rlmer(LDURATION~BEHAVIOR*SENTINEL_PRESENCE+BEHAVIOR*GENERALIZED_ENVIRONMENT+SENTINEL_PRESENCE*GENERALIZED_ENVIRONMENT+BEHAVIOR*GROUP_SIZE+BEHAVIOR*BAIT_PRESENCE+DISTURBANCE_FREQUENCY+(1|ID), data = BOUT)
...
```{r Post Hoc}
DIFF<-emmeans::emmeans(BOUT.MOD2, ~SENTINEL_PRESENCE*GENERALIZED_ENVIRONMENT+BAIT_PRESENCE,by = "BEHAVIOR", adjust = "fdr",pbkrtest.limit = 5070)
PAIRS<-emmeans::contrast(DIFF, "pairwise", adjust="fdr")
PAIRS
```
...
DF.DOT<-BOUT %>%
  ...
  scale_colour_manual(values = cbPalette
                      , guide="none")+
  theme(axis.title.x = element_text("Disturbance per Minute")
        , legend.position = "bottom")
