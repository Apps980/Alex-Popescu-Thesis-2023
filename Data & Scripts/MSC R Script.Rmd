...
>I removed observations past Sept 29 because I was told behaviors can change wildly in the winter. The number of crows spotted decreased past that date, suggesting the migrants have left or were leaving. To not have to include time of year (I already have part of the breeding season), I decided to simplify.\
...
#Removing outliers
Crow<-Crow[which(Crow$Duration>0.01),]
...
DATA$LOG_DURATION<-log(DATA$MEAN_BOUT_DURATION)
DATA$LOG_DURATION[is.infinite(DATA$LOG_DURATION)]<-NA
...
Untransformed.Dur<-hist(DATA$MEAN_BOUT_DURATION, breaks = 25)
Transformed.Dur<-hist(DATA$LOG_DURATION, breaks = 25)
```
...
Hist<-ggplot(data = DATA) +
  geom_histogram(aes(fill = BEHAVIOR, x = LOG_DURATION)
                 , alpha = 1
                 , bins = 50)+
...
source("./Calculate Summary Table with SE, SD, CI by grouping variables.R")
MeanDur <- summarySE(data = DATA, measurevar = "MEAN_BOUT_DURATION"
                     , groupvars = c("GENERALIZED_ENVIRONMENT", "BEHAVIOR", "SENTINEL_PRESENCE"))
DurDot<-ggplot(data = MeanDur
  ...
  geom_point(position = position_dodge2(width = 0.9)
             , size = 3) +
  geom_errorbar(aes(ymin=(MEAN_BOUT_DURATION-se)
                    , ymax=(MEAN_BOUT_DURATION+se))
                , width = 0.2
...
RLMM.Dur.I<-rlmer(LDur~Behavior+Sentinel*G.Env+Sentinel*CODED_NB_CROWS+Baited+Sentinel+G.Env+ (1|ID), data = Crow)
plot(RLMM.Dur.I)
```

```{r DATA Duration RLMM, echo=F}
RLMM.Dur.DATA<-rlmer(LOG_DURATION~BEHAVIOR+SENTINEL_PRESENCE*GENERALIZED_ENVIRONMENT+SENTINEL_PRESENCE*GROUP_SIZE+BAIT_PRESENCE+SENTINEL_PRESENCE+GENERALIZED_ENVIRONMENT+(1|ID), data = DATA)
plot(RLMM.Dur.DATA)
```
...
sjPlot::tab_model(RLMM.Dur.DATA
                  , show.re.var = T
...
BaitPlot<- Crow[which(Crow$Behavior=="HD"),] %>%
...
Transformed.PPM<-hist(DATA$LPek, breaks = 25)
```
...
```{r Peck Plots, echo = F}
PeckPlot<-ggplot(MeanPeck, aes(x = SENTINEL_PRESENCE, y = PECK_RATE, color=SENTINEL_PRESENCE))+
  geom_point(position = position_dodge(width = 0.9)
             , size = 3)+
...
I'll try to use a correlation test to determine if the presence of a sentinel (Yes/No, binary data) is correlated with the type of environment, the frequency of disturbances and the group size.

```{r Correlation test}
cor(DATA[, c("CODED_SENTINEL_PRESENCE", "CODED_ENV", "CODED_GROUP_SIZE", "TOTAL_FREQUENCY_OF_DISTURBANCES")])
```
